...
#   results/figures/KM_mgroup_full.png   (plot + risk table)
...
if (length(common_ids) < 50) stop(sprintf("[03] Too few common samples between meta2 and hall_df: %d", length(common_ids)))
...
if (sum(meta2$event == 1, na.rm = TRUE) < 5) {
  stop(sprintf("[03] Too few events after filtering. Events=1 count: %d",
               sum(meta2$event == 1, na.rm = TRUE)))
}

hall_df <- hall_df[rownames(meta2), , drop=FALSE]

# Ensure age exists if you want adjusted models
if ("age_years" %in% colnames(meta2)) {
  meta2$age_years <- as.numeric(meta2$age_years)
} else {
  message("[03] WARNING: age_years not found. Adjusted models will run without age_years.")
}
...
if (!is.na(stage4_name) && length(stage4_name) == 1) {
  meta2$stage_4 <- factor(as.character(meta2[[stage4_name]]))
} else {
  meta2$stage_4 <- NULL
}
...
if (length(top_pw) < 2) stop("[03] Too few top pathways available for LASSO after filtering.")
...
if (any(!is.finite(X_pw))) stop("[03] NA/Inf detected in X_pw. Please check hallmark scores (hall_df).")
...
if (length(sel_pw) == 0) {
  ...
  if (length(sel_1se) == 0) {
    stop("[03] LASSO selected 0 pathways with both lambda.min and lambda.1se. Expand features (top10/20) or use elastic-net.")
  } else {
...
# plot + table (nice for report)
ggsave(file.path(DIR$fig, "KM_mgroup_full.png"), print(p_km), width=7, height=8, dpi=300)

# ------------------------------
# 5) Adjusted Cox models
# ------------------------------
get_cindex <- function(fit) {
  mf <- model.frame(fit)
  y  <- model.response(mf)
  lp <- predict(fit, type="lp")
  sc <- survConcordance(y ~ lp)
...
# Model A (bin/bin)
vars_A <- c("pw_risk")
if ("age_years" %in% colnames(meta2)) vars_A <- c(vars_A, "age_years")
...
# Model B (4-level)
vars_B <- c("pw_risk")
if ("age_years" %in% colnames(meta2)) vars_B <- c(vars_B, "age_years")
...
# Compare AIC + C-index
cmp <- data.frame(model=character(), n=integer(), events=integer(), AIC=numeric(), C_index=numeric())
...
cat("===== Univariate hallmarks =====\n\n")
cat("Saved: results/tables/Cox_univariate_Hallmarks.csv\n\n")

cat("===== LASSO Cox =====\n")
cat("Top candidates:", paste(top_pw, collapse=", "), "\n")
...
cat("\n===== KM =====\n")
cat("Saved: results/figures/KM_mgroup.png (+ KM_mgroup_full.png)\n\n")

cat("===== Adjusted Cox models =====\n\n")
if (!is.null(models$cox_A_bin)) { cat("---- Model A (bin/bin) ----\n"); print(summary(models$cox_A_bin)); cat("\n") }
if (!is.null(models$cox_B_4level)) { cat("---- Model B (4-level) ----\n"); print(summary(models$cox_B_4level)); cat("\n") }

cat("===== Model comparison =====\n")
print(cmp)
sink()

message("[03] DONE: Cox + LASSO + KM + adjusted models saved.")
